...
def show():
    ...
    # Generate MCQ questions from the LLM pipeline
    if "grammar_questions" not in st.session_state:
        generated_questions = []
        for item in sentences:
            topic = item.get("topic")
            sentence_text = item.get("sentence") or item.get("question")
            if not sentence_text or not topic:
                continue

            mcq = generate_grammar_question(topic=topic, sentence=sentence_text)
            if mcq:
                generated_questions.append({
                    ...
                    "answer": mcq.answer,
                    "topic": topic,
                    "category": chosen_category or item.get("category", "general"),
                })

        st.session_state["grammar_questions"] = generated_questions
